function W = emgr(f,g,s,t,w,pr=0.0,nf=0,ut=1.0,us=0.0,xs=0.0,um=1.0,xm=1.0)
% emgr - Empirical Gramian Framework ( Version: 3.5.oct )
    ...
    switch(w) % by empirical gramian types
        ...
        case 'c', % controllability gramian
            for q=1:Q
                ...
                for c=1:C
                    for j=1:J % parfor
                        uu = us + (ut.*sparse(j,1,um(j,c),J,1));
            ...
            for q=1:Q
                ...
                for c=1:C
                    for j=1:J % parfor
                        ...
                        x -= res(x);
                        x *= 1.0./(um(j,c) + (um(j,c)==0));
                        uu = us + (ut.*sparse(j,1,xm(j,c),J,1));
