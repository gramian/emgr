...
# emgr - Empirical Gramian Framework ( Version: 3.1.oct )
...
#*
    ...
    # Version Info
    if(nargin==1 && strcmp(f,'version')), W = 3.1; return; endif;
    ...
    M = N; if(numel(s)==4), M = s(4); endif; # number of non-constant states
...
## GRAMIAN COMPUTATION
        ...
        case 'x', # cross gramian
            ...
            for q=1:Q
                ...
                for d=1:D
                    ...
                    if(M<N), pp = xs(M+1:end); endif;
                    for c=1:C
                        for j=1:J # parfor
                            uu = us + bsxfun(@times,ut,sparse(j,1,um(j,c),J,1));
                            x = ode(f,1,h,T,xs(1:M),uu,pp);
                            x = (x-res(x))*(1.0/um(j,c));
                            if(nf(10)), K = 1:O; else K = j; endif;
                            for k=K
                                W += (x*o(:,:,k)); # huma
                            endfor;
                        endfor;
    ...
    if(nf(11) && (w=='c'||w=='o'||w=='x'||w=='y') ), W = 0.5*(W+W'); endif;
end
...
function x = irk3(f,g,h,T,z,u,p)
    ...
    k1 = h*f(z,u(:,1),p); # Midpoint RK2 for starting value
