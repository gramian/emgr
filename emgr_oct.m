...
#  [Software]. Available from https://gramian.de . doi:110.5281/zenodo.2530021
#
# SEE ALSO:
#
#  gram
...
# Further information: https://gramian.de
    ...
    # Version Info
    if(strcmp(f,"version")), W = 5.6; return; endif
...
## GENERAL SETUP
    ...
    if(isscalar(xm)), xm = repmat(xm,N-((w=="y")*(N-M)),1); endif

    if(size(um,2)==1), um = scales(um,nf(2),nf(4)); endif
    if(size(xm,2)==1), xm = scales(xm,nf(3-(w=="y")),nf(5-(w=="y"))); endif
    ...
    if( (w=="c" || w=="o" || w=="x" || w=="y") && nf(6) && A==0)
        ...
        switch(nf(6))
            ...
            case 1 # Jacobi-type preconditioner
                ...
                WT = emgr_oct(f,g,s,t,w,pr,nf.*(not(1:12==6)),ut,us,xs,um,xm,DP);
                TX = sqrt(abs(WT));
