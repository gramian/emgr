function test_dwj(o)
% test_dwj (distributed cross-identifiability gramian parameter reduction)
...
%% OFFLINE
    ...
    tic;
    WJ = emgr(LIN,OUT,[J,N,O],T,'j',[zeros(N,1),ones(N,1)]);
    [UU,D,VV] = svd(WJ{2});
    OFFLINE = toc
    ...
    for I=1:n
        DWX = [w,I];
        wj = [wj,emgr(LIN,OUT,[J,N,O],T,'j',[zeros(N,1),ones(N,1)])];
    end;
    wx = wj(:,1:N);
